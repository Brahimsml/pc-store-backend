...
/* =======================
   DB CONNECTION (Railway + Local)  ✅ USE POOL
   Railway variable name = MYSQL_URL
======================= */
let db;
...
if (process.env.MYSQL_URL) {
  const uri = new URL(process.env.MYSQL_URL);
  ...
  db = mysql.createPool({
    ...
    database: uri.pathname.replace("/", ""), // usually "railway"
  ...
  console.log("✅ Using Railway MYSQL_URL (POOL)");
} else {
  db = mysql.createPool({
    host: "localhost",
    user: "root",
...
db.query("SELECT 1 AS ok", (err) => {
  if (err) console.error("❌ DB startup test failed:", err.message);
  else console.log("✅ DB startup test OK");
});
...
app.post("/admin/login", (req, res) => {
  ...
  db.query(q, [email, password], (err, data) => {
    ...
    const token = jwt.sign({ id: data[0].id, email: data[0].email }, SECRET, {
      expiresIn: "1h",
    });
...
app.post("/places/modify/:id", verifyAdmin, upload.single("image"), (req, res) => {
  ...
  db.query(q, [name, address, tel, image, req.params.id], (err) => {
    if (err) return res.status(500).json(err);
...
app.post("/products", verifyAdmin, upload.single("image"), (req, res) => {
  ...
  db.query(q, [category_id, name, price, image, label || null, description], (err, data) => {
    if (err) return res.status(500).json(err);
    res.json({ message: "Product added ✅", id: data.insertId });
  });
});
...
app.post("/products/modify/:id", verifyAdmin, upload.single("image"), (req, res) => {
  ...
  db.query(q, [category_id, name, price, image, label || null, description, req.params.id], (err) => {
    if (err) return res.status(500).json(err);
...
app.post("/orders", (req, res) => {
  ...
  const total = items.reduce((sum, it) => sum + cleanPrice(it.price) * (it.qty || 1), 0);
  ...
  db.query(qOrder, [customer_name, customer_phone, customer_address, total], (err, result) => {
    ...
    const values = items.map((it) => [orderId, it.id, it.name, cleanPrice(it.price), it.qty || 1]);
...
app.post("/contact", (req, res) => {
  ...
  if (!fname || !email || !message) return res.status(400).json({ message: "All fields are required" });
    ...
    db.query(qPlaces, (err2, r2) => {
      ...
      db.query(qOrders, (err3, r3) => {
        ...
        db.query(qContacts, (err4, r4) => {
          ...
          db.query("SELECT id, customer_name, total, status, created_at FROM orders ORDER BY id DESC LIMIT 5",
            (err5, recentOrders) => {
              if (err5) return res.status(500).json(err5);

              db.query("SELECT id, full_name, email, created_at FROM contacts ORDER BY id DESC LIMIT 5",
                (err6, recentMessages) => {
                  if (err6) return res.status(500).json(err6);
                  res.json({ stats, recentOrders, recentMessages });
                }
              );
            }
          );
